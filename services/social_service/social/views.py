...
def feed(request):
    ...
    posts_qs = Post.objects.annotate(avg_rating=Avg(
        'review__stars')).order_by("-created_at")

    if category:
        cat_posts = list(posts_qs.filter(category=category)[offset:offset + 5])
        other_posts = list(posts_qs.exclude(
            category=category)[offset:offset + 5])
    ...
    for post in selected:
        ...
        data["avg_rating"] = round(
            post.avg_rating, 2) if post.avg_rating else 0
...
def page_detail(request, store_id):
    ...
    posts = Post.objects.filter(store_id=store_id).annotate(
