...
A *UUID* stands for *Universally Unique Identifier*. It is a *128-bit identifier designed to be unique across
space and time*, even if it is generated on different machines without coordination.
...
The key idea is that *UUIDs aren’t guaranteed unique by checking a database* — they are *so
astronomically unlikely to collide that we treat them as unique*.
